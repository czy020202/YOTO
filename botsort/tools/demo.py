...
def imageflow_demo(vis_folder, current_time, args):
    ...
    timestamp = time.strftime("%Y_%m_%d_%H_%M_%S", current_time)
    save_folder = osp.join(vis_folder, filename)
    os.makedirs(save_folder, exist_ok=True)
    if args.demo == "video":
    ...
    while True:
        ...
        if ret_val:
            ...
            if filename + '_' + str(k) + '.txt' in os.listdir('../YOLOv5_zj/runs/detect/' + filename + '/labels/'):
                bboxes = np.loadtxt('../YOLOv5_zj/runs/detect/' + filename + '/labels/' + filename + '_' + str(k) + '.txt')
                bboxes = bboxes.reshape(-1, 6).tolist()
            else:
                bboxes = []
            bbox_xywh = []
    ...
    if args.save_result:
        res_file = osp.join(save_folder, f"{filename}.txt")
        with open(res_file, 'w') as f:
            f.writelines(results)
        logger.info(f"save results to {res_file}")
...
def main(exp, args):
    ...
    if args.demo == "image" or args.demo == "images":
        ...
        imageflow_demo(vis_folder, current_time, args)
    else:
